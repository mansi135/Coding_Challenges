...
def find(board, word):
    ...
    def helper(i, j, letter):
        ...
        if i-1 >= 0 and my_board[i-1][j] == letter[0]:
            #print my_board[i-1][j], "row-1"
            my_board[i-1][j] = 'seen'
            result = helper(i-1, j, letter[1:])
        if result is not True:
            if i+1 != len(my_board) and my_board[i+1][j] == letter[0]:
                #print my_board[i + 1][j], "row+1"
                my_board[i+1][j] = 'seen'
                result = helper(i+1, j, letter[1:])
        if result is not True:
            if j-1 >= 0 and my_board[i][j-1] == letter[0]:
                #print my_board[i][j-1], "col-1"
                my_board[i][j-1] = 'seen'
                result = helper(i, j-1, letter[1:])
        if result is not True:
            if j+1 != len(my_board[0]) and my_board[i][j+1] == letter[0]:
                #print my_board[i][j+1], "col+1"
                my_board[i][j+1] = 'seen'
                result = helper(i, j+1, letter[1:])
    ...
    for i in range(len(my_board)):
        ...
        for j in range(len(my_board[i])):
            #print j
            #print my_board[i][j], letters[0]
            if my_board[i][j] == letters[0]:
                my_board[i][j] = 'seen'
                #print my_board[i][j]
                word_exists = helper(i, j, letters[1:])
                if word_exists:
                    return True
